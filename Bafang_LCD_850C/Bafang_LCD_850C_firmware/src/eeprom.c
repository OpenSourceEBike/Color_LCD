...
void eeprom_init (void)
{
  ...
  // we must first init the emulated EEPROM
  eeprom_emulated_init();
  ...
  ui8_data = eeprom_emulated_read(ADDRESS_KEY);
...
static void eeprom_read_values_to_variables (void)
{
  ...
  p_configuration_variables->ui8_assist_level = eeprom_emulated_read(ADDRESS_ASSIST_LEVEL);

  ui16_temp = eeprom_emulated_read(ADDRESS_WHEEL_PERIMETER_0);
  ui8_temp = eeprom_emulated_read(ADDRESS_WHEEL_PERIMETER_1);
  ui16_temp += (((uint16_t) ui8_temp << 8) & 0xff00);
  p_configuration_variables->ui16_wheel_perimeter = ui16_temp;

  p_configuration_variables->ui8_wheel_max_speed = eeprom_emulated_read(ADDRESS_MAX_WHEEL_SPEED);
  p_configuration_variables->ui8_units_type = eeprom_emulated_read(ADDRESS_UNITS_TYPE);

  ui32_temp = eeprom_emulated_read(ADDRESS_HW_X10_OFFSET_0);
  ui8_temp = eeprom_emulated_read(ADDRESS_HW_X10_OFFSET_1);
  ui32_temp += (((uint32_t) ui8_temp << 8) & 0xff00);
  ui8_temp = eeprom_emulated_read(ADDRESS_HW_X10_OFFSET_2);
  ui32_temp += (((uint32_t) ui8_temp << 16) & 0xff0000);
  ui8_temp = eeprom_emulated_read(ADDRESS_HW_X10_OFFSET_3);
  ui32_temp += (((uint32_t) ui8_temp << 24) & 0xff000000);
  p_configuration_variables->ui32_wh_x10_offset = ui32_temp;

  ui32_temp = eeprom_emulated_read(ADDRESS_HW_X10_100_PERCENT_OFFSET_0);
  ui8_temp = eeprom_emulated_read(ADDRESS_HW_X10_100_PERCENT_OFFSET_1);
  ui32_temp += (((uint32_t) ui8_temp << 8) & 0xff00);
  ui8_temp = eeprom_emulated_read(ADDRESS_HW_X10_100_PERCENT_OFFSET_2);
  ui32_temp += (((uint32_t) ui8_temp << 16) & 0xff0000);
  ui8_temp = eeprom_emulated_read(ADDRESS_HW_X10_100_PERCENT_OFFSET_3);
  ui32_temp += (((uint32_t) ui8_temp << 24) & 0xff000000);
  p_configuration_variables->ui32_wh_x10_100_percent = ui32_temp;

  p_configuration_variables->ui8_show_numeric_battery_soc = eeprom_emulated_read(ADDRESS_SHOW_NUMERIC_BATTERY_SOC);
  p_configuration_variables->ui8_odometer_field_state = eeprom_emulated_read(ADDRESS_ODOMETER_FIELD_STATE);
  p_configuration_variables->ui8_battery_max_current = eeprom_emulated_read(ADDRESS_BATTERY_MAX_CURRENT);
  p_configuration_variables->ui8_target_max_battery_power = eeprom_emulated_read(ADDRESS_TARGET_MAX_BATTERY_POWER);
  p_configuration_variables->ui8_battery_cells_number = eeprom_emulated_read(ADDRESS_BATTERY_CELLS_NUMBER);

  ui16_temp = eeprom_emulated_read(ADDRESS_BATTERY_LOW_VOLTAGE_CUT_OFF_X10_0);
  ui8_temp = eeprom_emulated_read(ADDRESS_BATTERY_LOW_VOLTAGE_CUT_OFF_X10_1);
  ui16_temp += (((uint16_t) ui8_temp << 8) & 0xff00);
  p_configuration_variables->ui16_battery_low_voltage_cut_off_x10 = ui16_temp;

  ui8_temp = eeprom_emulated_read(ADDRESS_CONFIG_0);
  ...
  p_configuration_variables->ui8_number_of_assist_levels = eeprom_emulated_read(ADDRESS_NUMBER_OF_ASSIST_LEVELS);
  for (ui8_index = 0; ui8_index < 9; ui8_index++)
  {
    p_configuration_variables->ui8_assist_level_factor [ui8_index] = eeprom_emulated_read(ADDRESS_ASSIST_LEVEL_FACTOR_1 + ui8_index);
  }

  p_configuration_variables->ui8_startup_motor_power_boost_feature_enabled = eeprom_emulated_read(ADDRESS_STARTUP_MOTOR_POWER_BOOST_FEATURE_ENABLED);
  p_configuration_variables->ui8_startup_motor_power_boost_state = eeprom_emulated_read(ADDRESS_STARTUP_MOTOR_POWER_BOOST_STATE);
  p_configuration_variables->ui8_startup_motor_power_boost_time = eeprom_emulated_read(ADDRESS_STARTUP_MOTOR_POWER_BOOST_TIME);
  for (ui8_index = 0; ui8_index < 9; ui8_index++)
  {
    p_configuration_variables->ui8_startup_motor_power_boost_factor [ui8_index] = eeprom_emulated_read(ADDRESS_STARTUP_MOTOR_POWER_BOOST_ASSIST_LEVEL_1 + ui8_index);
  }

  p_configuration_variables->ui8_startup_motor_power_boost_fade_time = eeprom_emulated_read(ADDRESS_STARTUP_MOTOR_POWER_BOOST_FADE_TIME);

  p_configuration_variables->ui8_motor_temperature_min_value_to_limit = eeprom_emulated_read(ADDRESS_MOTOR_TEMPERATURE_MIN_VALUE_LIMIT);
  p_configuration_variables->ui8_motor_temperature_max_value_to_limit = eeprom_emulated_read(ADDRESS_MOTOR_TEMPERATURE_MAX_VALUE_LIMIT);

  ui16_temp = eeprom_emulated_read(ADDRESS_BATTERY_VOLTAGE_RESET_WH_COUNTER_X10_0);
  ui8_temp = eeprom_emulated_read(ADDRESS_BATTERY_VOLTAGE_RESET_WH_COUNTER_X10_1);
  ui16_temp += (((uint16_t) ui8_temp << 8) & 0xff00);
  p_configuration_variables->ui16_battery_voltage_reset_wh_counter_x10 = ui16_temp;

  p_configuration_variables->ui8_lcd_power_off_time_minutes = eeprom_emulated_read(ADDRESS_LCD_POWER_OFF_TIME);
  p_configuration_variables->ui8_lcd_backlight_on_brightness = eeprom_emulated_read(ADDRESS_LCD_BACKLIGHT_ON_BRIGHTNESS);
  p_configuration_variables->ui8_lcd_backlight_off_brightness = eeprom_emulated_read(ADDRESS_LCD_BACKLIGHT_OFF_BRIGHTNESS);

  ui16_temp = eeprom_emulated_read(ADDRESS_BATTERY_PACK_RESISTANCE_0);
  ui8_temp = eeprom_emulated_read(ADDRESS_BATTERY_PACK_RESISTANCE_1);
  ui16_temp += (((uint16_t) ui8_temp << 8) & 0xff00);
  p_configuration_variables->ui16_battery_pack_resistance_x1000 = ui16_temp;

  p_configuration_variables->ui8_offroad_feature_enabled = eeprom_emulated_read(ADDRESS_OFFROAD_FEATURE_ENABLED);
  p_configuration_variables->ui8_offroad_enabled_on_startup = eeprom_emulated_read(ADDRESS_OFFROAD_MODE_ENABLED_ON_STARTUP);
  p_configuration_variables->ui8_offroad_speed_limit = eeprom_emulated_read(ADDRESS_OFFROAD_SPEED_LIMIT);
  p_configuration_variables->ui8_offroad_power_limit_enabled = eeprom_emulated_read(ADDRESS_OFFROAD_POWER_LIMIT_ENABLED);
  p_configuration_variables->ui8_offroad_power_limit_div25 = eeprom_emulated_read(ADDRESS_OFFROAD_POWER_LIMIT_DIV25);

  ui32_temp = eeprom_emulated_read(ADDRESS_ODOMETER_X10_0);
  ui8_temp = eeprom_emulated_read(ADDRESS_ODOMETER_X10_1);
  ui32_temp += (((uint32_t) ui8_temp << 8) & 0xff00);
  ui8_temp = eeprom_emulated_read(ADDRESS_ODOMETER_X10_2);
  ui32_temp += (((uint32_t) ui8_temp << 16) & 0xff0000);
...
static void eeprom_write_array (uint8_t *p_array, uint8_t ui8_len)
{
  ...
  do
  {
    ...
    {
      eeprom_emulated_write(((uint32_t) EEPROM_BASE_ADDRESS) + ((uint32_t) ui8_i), *p_array_data++);
    }
...
void eeprom_erase_key_value (void)
{
  ...
  {
    // set to 0 the KEY
    eeprom_emulated_write(ADDRESS_KEY, 0);
