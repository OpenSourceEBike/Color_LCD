...
static void bootScreenOnUpdate() {
	...
	if (ui32_seconds_since_startup - start_time >= 5
			&& (has_seen_motor || is_sim_motor))
		showNextScreen();
}

Screen bootScreen = { .onUpdate = bootScreenOnUpdate,

.fields = { { .x = 0, .y = 0, .height = -1, .field = &bootHeading, .font =
		&REGULAR_TEXT_FONT, },
#ifndef SW102
		{ .x = 0, .y = -8, .height = -1, .field = &bootURL, .font =
				&SMALL_TEXT_FONT, },
#endif
		{ .x = 0, .y = YbyEighths(4), .height = -1, .field = &bootVersion,
				.font = &SMALL_TEXT_FONT, }, { .x = 0, .y = YbyEighths(6),
				.height = -1, .field = &bootStatus, .font = &SMALL_TEXT_FONT, },
		{ .field = NULL } } };
