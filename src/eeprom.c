...
static const uint32_t eeprom_magic = 0x5a;
static const uint32_t eeprom_invalid = 0; // no valid images in eeprom
...
void eeprom_init()
{
  uint32_t readmagic = eeprom_invalid;

  eeprom_hw_init();
  flash_read_words(0, &readmagic, 1);
  if(readmagic != eeprom_magic)
...
void eeprom_init_variables(void)
{
  ...
  flash_read_words(1, &m_eeprom_data, sizeof(m_eeprom_data) / sizeof(uint32_t));
...
void eeprom_write_variables(void)
{
  ...
  flash_write_words(1, &m_eeprom_data,  sizeof(m_eeprom_data) / sizeof(uint32_t));
  flash_write_words(0, &eeprom_magic, 1); // mark that eeprom image is valid
}
...
{
  flash_write_words(0, &eeprom_invalid, 1); // mark that eeprom image is valid
...
{
  flash_write_words(1, &m_eeprom_data_defaults,  sizeof(m_eeprom_data_defaults) / sizeof(uint32_t));
  flash_write_words(0, &eeprom_magic, 1); // mark that eeprom image is valid
}
